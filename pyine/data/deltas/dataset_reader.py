...
class DatasetReader(pyine.data.traces.dataset_reader.DatasetReader):
    ...
    def __init__(
        ...
        deltas_suffix = pyine.data.deltas.dataset_utils.DELTAS_SUFFIX
        self.deltas_keys = [d for d in self.trace_keys if d.endswith(deltas_suffix)]
        self.deltas_indices = [self.trace_indices[self.trace_keys.index(d)] for d in self.deltas_keys]
        assert len(self.deltas_indices) == len(self.deltas_keys)
        for dkey, didx in zip(self.deltas_keys, self.deltas_indices):
            self.trace_keys.remove(dkey)
            self.trace_indices.remove(didx)
        assert len(self.trace_indices) == len(self.deltas_indices)
