...
class EventRelationship(enum.StrEnum):
    ...
    # TODO: consider adding control block relationships or STEP_INTO interpretation options.
...
class TraceDelta:
    ...
    def __repr__(self) -> str:
        ...
        delta_payload = dict(self.variables)
        if self.stdout is not None:
            delta_payload["__stdout__"] = self.stdout
        if self.stderr is not None:
            delta_payload["__stderr__"] = self.stderr
        location_summary = (
            f"{curr_trace_file_prefix}L{self.curr_trace_key.line:04d} -> "
            f"{next_trace_file_prefix}L{self.next_trace_key.line:04d}"
        )
        return f"{location_summary} ({self.event_relationship}) @ step#{self.trace_step_idx} = {delta_payload}"
...
def _wrapped_delta_generator(
    ...
    if isinstance(curr, pyine.utils.code.execution.TraceEvent):
        curr = {**curr.global_variables, **curr.local_variables} if include_global_vars else curr.local_variables
    if isinstance(next, pyine.utils.code.execution.TraceEvent):
        next = {**next.global_variables, **next.local_variables} if include_global_vars else next.local_variables
...
class _CallStack:
    ...
    def init(self, curr_step: pyine.utils.code.execution.TraceEvent) -> pyine.utils.code.execution.TraceKey:
        ...
        caller_arguments: dict[str, typing.Any] = dict(curr_step.arguments or {})
        self._stack.append((self.orig_caller_trace_key, caller_arguments))
...
class _EventPairIterator:
    ...
    def _filter_relevant_source_trace_step_idxs(
        ...
        filtered_trace_step_idxs: list[int] = []
    ...
    def get_next_pair(
        ...
        if curr_step is None or next_step is None:
            raise AssertionError("expected trace events when building deltas")
    ...
    def get_next_event(
        ...
        if self.iter_idx < len(self.relevant_step_idxs) - 1:
            next_step_candidate = self.trace_res.traced_steps[self.relevant_step_idxs[self.iter_idx + 1]]
            if next_step_candidate is None:
                raise AssertionError("expected trace event when iterating")
            if increment:
                self.iter_idx += 1
...
def get_deltas_from_trace_steps(
    ...
    delta_generator_fn = simple_delta_generator if delta_generator == DeltaGeneratorType.SIMPLE else deepdiff.DeepDiff
    ...
    while event_iterator.has_next_event():  # as long as there are still event pairs to generate...
        ...
        if relationship == EventRelationship.RAISE:
            ...
            delta = {"__exception__": repr(raised_exception)}
            ...
            if next_next_step is None:
                raise AssertionError("unexpected end of trace when handling raised exception")
        ...
        if relationship in [EventRelationship.STEP_OUT, EventRelationship.RAISE]:
            ...
            if raised_exception:
                ...
                delta = {"__return__": repr(next_step.return_value)}
                output_relationship = EventRelationship.STEP_OUT
            if caller_trace_key == call_stack.orig_caller_trace_key:
            ...
            if event_iterator.has_next_event():  # we could be at the end of the trace
                ...
                if next_next_step is None:
                    raise AssertionError("unexpected end of trace when handling step out")
                ...
                # once we get here, we've stopped propagating because we exited tracing or found a catcher
                    ...
                    call_stack.push_existing(caller_trace_key, caller_vars)  # put orig caller info back on stack
                    next_call_delta = {
                    ...
                    if next_next_step is None:
                        raise AssertionError("expected follow-up trace event when stepping into call")
                    output_deltas.append(
                        TraceDelta(
        ...
        if relationship == EventRelationship.STEP_INTO:
            ...
            next_next_step = event_iterator.get_next_event(increment=True)  # fetch next event for its trace key
            if next_next_step is None:
                raise AssertionError("unexpected end of trace when stepping into function")
