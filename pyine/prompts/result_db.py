...
def _build_record_uid_prefix(
    identifier: str,
    prompt_name: PromptNameType | None,
    prompt_version: PromptVersionType | None,
    created_at: datetime.datetime,
) -> str:
    """Build the human-readable prefix shared by record_uid and summary record_uid_prefix."""
    parts = [identifier]
    if prompt_name:
        parts.append(str(prompt_name))
    if prompt_version:
        parts.append(str(prompt_version))
    parts.append(created_at.astimezone(datetime.UTC).strftime("%Y%m%d-%H%M%S"))
    return "_".join(parts)
...
class PromptResultRecord(pydantic.BaseModel):
    ...
    meta: dict[str, pydantic.JsonValue] = pydantic.Field(default_factory=lambda: {})
    ...
    @property
    def record_uid_prefix(self) -> str:
        """Human-readable prefix of the record UID (without the content hash suffix)."""
        return _build_record_uid_prefix(
            self.identifier,
            self.prompt_name,
            self.prompt_version,
            self.creation_meta.created_at,
        )
    ...
    def record_uid(self) -> str:
        ...
        return f"{self.record_uid_prefix}_{content_hash}"


class PromptResultSummary(pydantic.BaseModel):
    """Lightweight summary of a prompt result entry, excluding heavy text fields."""
    ...
    """Optional group name for grouping similar records together."""
    created_at: datetime.datetime
    """Timestamp of record creation (always UTC); parsed from the SQL created_at column."""
    meta: dict[str, pydantic.JsonValue] = pydantic.Field(default_factory=lambda: {})
    """Additional metadata associated with this record."""
    tags: list[str] = pydantic.Field(default_factory=lambda: [])
    """Tags associated with this record."""
    ...
    @property
    def record_uid_prefix(self) -> str:
        """Human-readable prefix matching PromptResultRecord.record_uid_prefix."""
        return _build_record_uid_prefix(
            self.identifier,
            self.prompt_name,
            self.prompt_version,
...
_SUMMARY_COLUMNS = 'identifier, "group", prompt_name, prompt_version, created_at, meta, tags'
"""SQL column list for lightweight summary queries (excludes prompt, result, creation_meta)."""
...
class PromptResultDB:
    ...
    @staticmethod
    def _row_to_summary(row: sqlite3.Row) -> PromptResultSummary:
        """Convert a sqlite3.Row (from a summary-columns query) to a PromptResultSummary."""
        ...
        return PromptResultSummary(
            identifier=row["identifier"],
            prompt_name=row["prompt_name"],
            prompt_version=row["prompt_version"],
            group=row["group"],
            created_at=created_at,
            meta=_reload_metadata(row["meta"]),
            tags=_reload_tags(row["tags"]),
        )
    ...
    def _get_summaries(
        ...
        params: list[typing.Any],
        tag_filter_rule: str | None,
    ) -> list[PromptResultSummary]:
        """Fetch lightweight summaries from the database using the provided SQL and params."""
        conn = self._connect(row_factory=True)
        try:
            rows = conn.execute(" ".join(sql), params).fetchall()
        ...
        if tag_filter_rule is not None:
            ...
            summaries = [s for s in summaries if not tag_filter_fn(s.tags or [])]
        return summaries

    def get_summaries_by_prompt_name(
        self,
        prompt_name: str,
        *,
        prompt_version: PromptVersionType | None = None,
        ...
        """Fetch lightweight summaries for all records matching the given prompt name.

        This is more efficient than get_by_prompt_name when the full prompt/result text is not
        needed, as it skips the heavy prompt, result, and creation_meta columns.

        Args:
            prompt_name: Prompt template name to filter by.
        ...
        Returns:
            List of matching PromptResultSummary objects, ordered by identifier and creation time.
        """
        if not prompt_name:
            raise ValueError("prompt name required")
        sql = [f"SELECT {_SUMMARY_COLUMNS} FROM items WHERE prompt_name = ?"]  # noqa: S608
        params: list[typing.Any] = [prompt_name]
        if prompt_version is not None:
            sql.append("AND prompt_version = ?")
            params.append(prompt_version)
        if max_result_age is not None:
            ...
            params.append(cutoff.isoformat())
        sql.append("ORDER BY identifier ASC, created_at ASC, id ASC")
        return self._get_summaries(sql, params, tag_filter_rule)
    ...
    def get_all_summaries(
        ...
        *,
        ...
        """Fetch lightweight summaries for all records in the database.
        ...
        Returns:
            List of matching PromptResultSummary objects, ordered by identifier and creation time.
        """
        sql = [f"SELECT {_SUMMARY_COLUMNS} FROM items WHERE 1=1"]  # noqa: S608
        params: list[typing.Any] = []
        if max_result_age is not None:
            cutoff = datetime.datetime.now(datetime.UTC) - max_result_age
            sql.append("AND created_at > ?")
