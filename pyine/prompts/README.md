...
- **Versioned prompts with defaults.** A single YAML can host many versions; a `__default__` key
  selects the default version that is loaded by the manager. If no default is specified, the last
  version defined in the file is taken as the default.
- **Strongly typed examples and outputs.** YAML examples can embed tagged Pydantic objects, which
  are automatically registered and instantiated when YAML files are loaded; your prompt unit tests
...
- **Simple discovery.** `list_prompts` and `list_prompt_versions` allow tooling and notebooks to
  discover all available prompts and their variants.

### Architecture and main concepts

#### Versioned YAML templates (`pyine/prompts/templates`)

Each file corresponds to a named prompt. Some prompts are grouped in subdirectories based what
prompt family they belong to (e.g., `hints/tests.yaml` is referred to as `hints/tests`).
...
v1.0-universal:  # arbitrary name that identifies the VERSION of the prompt
  metadata:  # required field that provides some human-readable information about the prompt
    name: code_analysis  # 'name' of the prompt that
...
- __defines__: mapping: reusable anchors/aliases (skipped by the parser and ignored by the loader).

**Example-specific conventions (enforced by utils):**

- `example_template` must accept an "output" variable (`EXAMPLE_OUTPUT_KEY`). That output does not
  however need to be a Pydantic model.
...
The `PromptConfig` class wires the four prompt blocks, manages examples, and constructs a LangChain
`PromptTemplate` object on demand. Important APIs include:

- `PromptConfig.create_prompt_template`: creates a LangChain `PromptTemplate` object from the
  role/context/examples/question blocks.
...
1. Add a YAML template file under `pyine/prompts/templates`:
   - Choose a file path; this path (minus `.yaml`) becomes the prompt name, for example:
     `some_group/my_prompt.yaml` should have `"some_group/my_prompt"` as its prompt name.
   ...
   - Optionally add `role`, `context`, `example_template`, `examples`, `examples_block_template`, and
     `template_block_separator` fields. If including examples, ensure `example_template` contains an
     "output" variable. With jinja2 you can write `{{output}}` or `{{output.model_dump_json()}}` if
     it’s a Pydantic object. You may reference `{{example_idx}}` and `{{example_count}}` in the
     example template; the engine fills them in when requested.
2. (Optional but recommended) Add a config module under `pyine/prompts/configs`.
   ...
   - Define Pydantic models to structure outputs and examples (they will be auto-registered).
   - Expose a `get_prompt_config(version: str | None = None) -> PromptConfig` function with any kind
     of custom handling your implementation might require.
   - Expose a `get_prompt_template(...) -> LangChain PromptTemplate` function, and optionally inject
     computed partial variables (e.g., structured output format instructions).
3. Write tests
   - See tests/prompts for patterns; they should cover discovery, prompt listing, defaults, and basic
     retrieval.

### Conventions and tips

**Template formats.** Prefer f-string format for safety. Use jinja2 only when you need logic
(e.g., conditional blocks) and control the input variables.
...
(e.g. "v1.0"), provider/model-targeted versions (openai/gpt-4o-2024-xx), or hierarchical names
(full_output/v1.0). The key only has to match exactly when requested.
