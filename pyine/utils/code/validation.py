...
def validate_code(code_string: str, max_size: int = 100_000) -> None:
    ...
    for node in ast.walk(parsed_tree):
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            ...
            isinstance(node, ast.While) and isinstance(node.test, ast.Constant) and bool(node.test.value)
        ):  # check for infinite while loop
            has_exit = any(isinstance(inner, (ast.Break, ast.Return)) for inner in ast.walk(node))
            assert has_exit, "found potentially infinite while loop without break or return"
