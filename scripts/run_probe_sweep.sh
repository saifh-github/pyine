#!/usr/bin/env bash
# run_probe_sweep.sh: launches probe training jobs sequentially on GPUs 0-3.
set -euo pipefail

export CUDA_VISIBLE_DEVICES=0,1,2,3
export WANDB_PROJECT=pyine-guardrails

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUN_DDP="${SCRIPT_DIR}/run_ddp.sh"

JOBS=(
    "--app pyine.apps.trainers.probe_trainer --nproc_per_node 4 -- +experiment=probes/v0_probe"
    ...
    # "--app pyine.apps.trainers.probe_trainer --nproc_per_node 4 -- +experiment=probes/TODO_job4"
)
