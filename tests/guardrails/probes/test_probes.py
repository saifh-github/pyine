...
def _make_probe(
    architecture: str, extra_kwargs: dict, hidden_dim: int = tests.guardrails.probes.conftest.PROBE_HIDDEN_DIM
    ...
    return pyine.guardrails.probes.build_probe(config)
...
class TestProbeArchitectures:
    def test_output_shape(
        ...
        assert out.shape == (tests.guardrails.probes.conftest.PROBE_BATCH_SIZE, 1)
...
class TestBuildProbe:
    def test_build_probe_all_architectures(self) -> None:
        for arch_name, expected_cls in pyine.guardrails.probes.PROBE_REGISTRY.items():
            config = pyine.guardrails.probes.base.ProbeConfig(
                name=f"test_{arch_name}", architecture=arch_name, layer=0, hidden_dim=32
            )
            probe = pyine.guardrails.probes.build_probe(config)
            assert isinstance(probe, expected_cls)

    def test_build_probe_unknown_architecture_raises(self) -> None:
        config = pyine.guardrails.probes.base.ProbeConfig(
            name="bad", architecture="nonexistent", layer=0, hidden_dim=32
        )
        with pytest.raises(KeyError, match="nonexistent"):
            pyine.guardrails.probes.build_probe(config)

    def test_build_probe_missing_hidden_dim_raises(self) -> None:
        config = pyine.guardrails.probes.base.ProbeConfig(name="bad", architecture="mean", layer=0)
