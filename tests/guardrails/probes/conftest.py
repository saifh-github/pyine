...
@pytest.fixture
def sample_probe_configs() -> list[pyine.guardrails.probes.base.ProbeConfig]:
    ...
    return [
        ...
        pyine.guardrails.probes.base.ProbeConfig(name="max_L0", architecture="max", layer=0),
        pyine.guardrails.probes.base.ProbeConfig(name="last_L4", architecture="last_token", layer=4),
        pyine.guardrails.probes.base.ProbeConfig(
            name="rolling_L4", architecture="rolling_mean", layer=4, window_size=4
        ),
        pyine.guardrails.probes.base.ProbeConfig(name="softmax_L8", architecture="softmax", layer=8, temperature=0.5),
        pyine.guardrails.probes.base.ProbeConfig(name="attn_L8", architecture="attention", layer=8, attn_dim=16),
    ]


@pytest.fixture
def replica_probe_configs() -> list[pyine.guardrails.probes.base.ProbeConfig]:
...
def replica_probe_collection(
    ...
    return pyine.guardrails.probes.collection.ProbeCollection(replica_probe_configs, hidden_dim=PROBE_HIDDEN_DIM)
...
def debug_lmdb_path(tmp_path: pathlib.Path) -> pathlib.Path:
    ...
    pyine.guardrails.data.debug_dataset.create_debug_probe_lmdb(lmdb_path, n_train=50, n_valid=20, seed=42)
    return lmdb_path
